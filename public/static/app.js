        ...
        <div class="question-navigation">
            ...
            <div class="question-indicator">
                ${Array.from({length: 10}, (_, i) => `
                    <span class="indicator-dot ${i === currentQuestionIndex ? 'active' : ''} ${userAnswers[currentQuestions[i].id_soal] ? 'answered' : ''}"
...
function updateSessionInfoWithQuestion(q) {
    ...
    sessionInfoRight.innerHTML = `
        <div class="question-badge" id="questionBadge">Soal ${currentQuestionIndex + 1} dari 10</div>
    ...
    if (data.passed) {
            ...
            reviewHtml += `
                <div class="explanation-card ${isCorrect ? 'correct' : 'incorrect'}">
                    <div class="explanation-header">
                        <strong>${isCorrect ? '✅' : '❌'} Soal ${index + 1}</strong>
                        <span class="question-topic-small">${question.materi}</span>
                    </div>
                    <div class="explanation-question">${question.pertanyaan}</div>
                    <div class="explanation-answers">
                        <div class="answer-row ${isCorrect ? 'correct' : 'wrong'}">
                            <span class="answer-label">Jawaban Anda:</span>
                            <span class="answer-value">${result.user_answer || 'Tidak dijawab'} ${isCorrect ? '✓' : '✗'}</span>
                        </div>
                        ${!isCorrect ? `
                            <div class="answer-row correct">
                                <span class="answer-label">Jawaban Benar:</span>
                                <span class="answer-value">${result.correct_answer}. ${question['pilihan_' + result.correct_answer.toLowerCase()]}</span>
                            </div>
                        ` : ''}
                    </div>
                    ${question.penjelasan ? `
        ...
        resultCard.innerHTML = `
            <div class="text-center">
                ...
                <div class="score-display">
                    <h3>${data.score} / ${data.total}</h3>
                    <p>Skor: ${(data.score / data.total * 100).toFixed(0)}%</p>
                </div>
...
function updateProgressBar() {
    ...
    document.getElementById('progressText').textContent = `Sesi ${currentSession} dari 20`;
}

// Update session info
function updateSessionInfo(sessionId) {
    document.getElementById('sessionTitle').textContent = `Sesi ${sessionId} - 10 Soal`;
    const startQ = (sessionId - 1) * 10 + 1;
    const endQ = sessionId * 10;
    document.getElementById('sessionDesc').textContent = 
        `Soal ${startQ} - ${endQ} | Jawab semua dengan benar untuk melanjutkan`;
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key >= '1' && e.key <= '4') {
        ...
        const radioButton = document.getElementById(`q${currentQuestion.id_soal}_${selectedOption}`);
        if (radioButton) {
